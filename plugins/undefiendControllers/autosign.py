...
async def _():
    try:
        with open(DATA_PATH + "config.toml", "r", encoding="utf-8") as f:
            config = toml.load(f)
        if config["EnableAutoSign"]:
            ...
            bot.send_private_msg(user_id = eval(open("./.env.prod", "r").readlines() [4].replace("SUPERUSERS=", ""))[0], message = f"TLoH Bot - 自动签到\n已完成自动签到。签到 {len(group_list)} 个群组。")
            # 解析：
